/**
 * gen-legendary-extensions.js
 *
 * MCH エクステンション CSV (Legendary) から本タイトル用カード定義を生成。
 * gen-epic-extensions.js の Legendary 版。
 *
 * 使い方:
 *   node prototype/tools/gen-legendary-extensions.js <input.csv> > out.txt
 *
 * 入力 CSV (bearko/mycryptoheroes/Data/Extensions/extensions.csv) のうち
 * Rarity=Legendary の行を対象にする。
 *
 * 重要: extId >= 5501 は LL エクステンション (本プロジェクトでは別系統データ) として
 * 除外する。対象は extId < 5501 の Legendary 178 件のみ。
 *
 * Legendary カードは Epic より強力にスケーリング:
 * - ステータス上昇 flat: Common +1〜+3 / Uncommon +1〜+4 / Rare +1〜+5 / Epic +1〜+6 / Legendary +1〜+7
 * - ステータス減少 flat: Common -1〜-2 / Uncommon -1〜-3 / Rare -1〜-4 / Epic -1〜-5 / Legendary -1〜-6
 * - 状態異常 stack 数: Common 1 / Uncommon 1 / Rare 2 / Epic 3 / Legendary 4
 * - cost: 高%ダメージや multi-hit は cost 2-3、それ以外は cost 1-2
 */
const fs = require("fs");
...
function parseEffects(text, rarity) {
  ...
  function scaleStat(pct) {
    const cap = _rarity === "legendary" ? 7 : (_rarity === "epic" ? 6 : (_rarity === "rare" ? 5 : (_rarity === "uncommon" ? 4 : 3)));
    return Math.max(1, Math.min(cap, Math.floor(pct / 10)));
  }
  function scaleDebuff(pct) {
    const cap = _rarity === "legendary" ? 6 : (_rarity === "epic" ? 5 : (_rarity === "rare" ? 4 : (_rarity === "uncommon" ? 3 : 2)));
    return -Math.max(1, Math.min(cap, Math.floor(pct / 10)));
  }
  ...
  // Rarity 別の stack 数: Common/Uncommon 1 / Rare 2 / Epic 3 / Legendary 4
  const stackBase = _rarity === "legendary" ? 4 : (_rarity === "epic" ? 3 : (_rarity === "rare" ? 2 : 1));
...
function main() {
  ...
  const rows = lines.slice(1).map(l => {
    ...
    return obj;
  }).filter(r => r.id && r.rarity === "Legendary" && +r.id < 5501);  // ext >= 5501 は LL ext として除外
  ...
  console.log("// ─── auto-generated Legendary extensions (" + generated.length + " cards、ext>=5501 LL ext は除外) ───");
