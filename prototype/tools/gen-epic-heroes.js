 ...
 * gen-epic-heroes.js
 *
 * MCH ヒーロー CSV (Epic) から本タイトル用のヒーロー定義 + パッシブ関数を生成。
 * gen-rare-heroes.js の Epic 版（スケーリング強化）。
 *
 * 使い方:
 *   node prototype/tools/gen-epic-heroes.js <input.csv> > out.txt
 *
 * 出力:
 *   1) heroes.json に append する 61 エントリ
 *   2) main.js に append するパッシブ関数 + dispatcher case 句
 *
 * Epic スケーリング:
 *   - ステータス上昇 cap: +1〜+7 (vs Rare +1〜+6 / Uncommon +1〜+5 / Common +1〜+3)
 *   - ステータス減少 cap: -1〜-6 (vs Rare -1〜-5 / Uncommon -1〜-4)
 *   - 状態異常 stack: 3 (vs Rare 2 / Common+Uncommon 1)
 *   - HP 範囲: 95-120 (vs Rare 90-115)
 *   - PHY/INT/AGI 範囲: 7-26 (vs Rare 6-24)
...
// Epic はヒーローとしては Rare より少し上のスタッツ範囲
// (HP 95-120, P/I/A 7-26) に。
function scaleStats(hp, phy, int_, agi) {
  // HP scaling: target range 95-120
  const hpMax = Math.max(95, Math.min(120, Math.round(hp / 4)));
  // PHY/INT/AGI: divide by 7, clamp to 7-26
  const sP = phy === 0 ? 0 : Math.max(7, Math.min(26, Math.round(phy / 7)));
  const sI = int_ === 0 ? 0 : Math.max(7, Math.min(26, Math.round(int_ / 7)));
  const sA = agi === 0 ? 0 : Math.max(7, Math.min(26, Math.round(agi / 7)));
...
function parsePassive(text) {
  ...
  // Self stat buffs (Epic: cap at +1〜+7)
  ...
  // Status effects (project has poison/bleed only); Epic は stack 数 3
  if (text.includes("毒")) e.poison += 3;
  if (text.includes("出血")) e.bleed += 3;
  if (text.includes("気絶") || text.includes("睡眠")) e.bleed += 3;
  if (text.includes("衰弱") || text.includes("恐怖") || text.includes("呪い") ||
      text.includes("混乱") || text.includes("バインド")) e.poison += 3;
