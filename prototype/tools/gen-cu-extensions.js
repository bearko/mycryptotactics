/**
 * gen-cu-extensions.js
 *
 * MCH エクステンション CSV (Common + Uncommon) から本タイトル用カード定義を生成。
 *
 * 使い方:
 *   node prototype/tools/gen-cu-extensions.js <input.csv> > out.txt
 *
 * 入力 CSV (bearko/mycryptoheroes/Data/Extensions/extensions.csv) のうち
 * Rarity=Common|Uncommon の行を対象にする。
 *
 * 出力:
 *   1) cards.js makeCardLibrary 内に append する `extXXXX: { ... },` ブロック
 *   2) CARD_RARITIES に追記する `extXXXX: 'common',` または `'uncommon',` 行
 *
 * 効果は元 DB の active_skill_text_ja を解析して可能な限り保持。
 * 解析できないもの・本タイトルに無いもの (party 系・状態異常複数等) は
 * StS 風代替効果に差し替える。スキル名・名前は変更しない。
 */
const fs = require("fs");
const path = require("path");

// 既に cards.js で定義済みの extId はスキップする
const EXISTING_EXT_IDS = new Set([
  1001, 1002, 1003, 1004, 1005, 1006, 1007, 1008, 1011, 1012,
  1022, 1023, 2001, 2002, 2003, 2004, 2005, 2006, 2008, 2011, 2013,
]);
...
function parseEffects(text, rarity) {
  ...
  if (text.includes("気絶") || text.includes("睡眠")) e.bleed += 1;       // stun-like → bleed
  if (text.includes("衰弱") || text.includes("恐怖") ||
      text.includes("呪い") || text.includes("混乱")) e.poison += 1;     // debuff-like → poison
...
function main() {
  ...
  const rows = lines.slice(1).map(l => {
    ...
    return obj;
  }).filter(r => r.id && r.rarity && (r.rarity === "Common" || r.rarity === "Uncommon"));
  ...
  console.log("// ─── auto-generated CU extensions (Common+Uncommon, " + generated.length + " cards) ───");
