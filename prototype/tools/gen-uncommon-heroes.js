/**
 * gen-uncommon-heroes.js
 *
 * MCH ヒーロー CSV (Uncommon) から本タイトル用のヒーロー定義 + パッシブ関数を生成。
 *
 * 使い方:
 *   node prototype/tools/gen-uncommon-heroes.js <input.csv> > out.txt
 *
 * 出力:
 *   1) heroes.json に append する 53 エントリ
 *   2) main.js に append するパッシブ関数 + dispatcher case 句
 *
 * Common ヒーロー実装（PR #50）と同様の方針:
 ...
 * - 元の effect は可能な限り保持、本タイトルにない要素 (パーティ, 多彩な状態異常) は単純化
 */
const fs = require("fs");
...
// ─── スタッツスケール ──────────────────────────────────────────
// MCH オリジナルの数値を本タイトル範囲 (HP 85-110, P/I/A 5-22) に圧縮。
function scaleStats(hp, phy, int_, agi) {
  // HP scaling: rough divider, target range 85-110
  const hpMax = Math.max(85, Math.min(110, Math.round(hp / 4)));
  // PHY/INT/AGI: divide by 7, clamp to 5-22, ensure max 1 if input > 0
  const sP = phy === 0 ? 0 : Math.max(5, Math.min(22, Math.round(phy / 7)));
  const sI = int_ === 0 ? 0 : Math.max(5, Math.min(22, Math.round(int_ / 7)));
  const sA = agi === 0 ? 0 : Math.max(5, Math.min(22, Math.round(agi / 7)));
...
function parsePassive(text) {
  ...
  // Status effects (project has poison/bleed only)
  if (text.includes("毒")) e.poison += 1;
  if (text.includes("出血")) e.bleed += 1;
  if (text.includes("気絶") || text.includes("睡眠")) e.bleed += 1;
  if (text.includes("衰弱") || text.includes("恐怖") || text.includes("呪い") ||
      text.includes("混乱") || text.includes("バインド")) e.poison += 1;
...
function main() {
  ...
  const rows = lines.slice(1).map(l => {
    ...
    return obj;
  }).filter(r => r.id && r.rarity === "Uncommon");
