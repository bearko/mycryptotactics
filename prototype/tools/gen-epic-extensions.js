/**
 * gen-epic-extensions.js
 *
 * MCH エクステンション CSV (Epic) から本タイトル用カード定義を生成。
 * gen-rare-extensions.js の Epic 版。
 *
 * 使い方:
 *   node prototype/tools/gen-epic-extensions.js <input.csv> > out.txt
 *
 * 入力 CSV (bearko/mycryptoheroes/Data/Extensions/extensions.csv) のうち
 * Rarity=Epic の行を対象にする。
 *
 * Epic カードは Rare より強力にスケーリング:
 * - ステータス上昇 flat: Common +1〜+3 / Uncommon +1〜+4 / Rare +1〜+5 / Epic +1〜+6
 * - ステータス減少 flat: Common -1〜-2 / Uncommon -1〜-3 / Rare -1〜-4 / Epic -1〜-5
 * - 状態異常 stack 数: Common 1 / Uncommon 1 / Rare 2 / Epic 3
...
function parseEffects(text, rarity) {
  ...
  function scaleStat(pct) {
    const cap = _rarity === "epic" ? 6 : (_rarity === "rare" ? 5 : (_rarity === "uncommon" ? 4 : 3));
    return Math.max(1, Math.min(cap, Math.floor(pct / 10)));
  }
  function scaleDebuff(pct) {
    const cap = _rarity === "epic" ? 5 : (_rarity === "rare" ? 4 : (_rarity === "uncommon" ? 3 : 2));
    return -Math.max(1, Math.min(cap, Math.floor(pct / 10)));
  }
  ...
  // Rarity 別の stack 数: Common/Uncommon 1 / Rare 2 / Epic 3
  const stackBase = _rarity === "epic" ? 3 : (_rarity === "rare" ? 2 : 1);
...
function main() {
  ...
  console.log("// ─── auto-generated Epic extensions (" + generated.length + " cards) ───");
