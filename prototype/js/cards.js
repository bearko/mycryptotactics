...
/** カードシリーズ情報 (auto-generated by tools/gen-card-series.js)
 *  元データ: bearko/mycryptoheroes/Data/Extensions/extensions.json の series.name.ja
 *  CARD_RARITIES と合わせて findUpgradeKey() の動的計算に使う。 */
export const CARD_SERIES = {
  ...
  ext1136: 'ビートル',
  ext1137: 'ハチ',
  ext1138: 'Sweet Fluffy and chewy',
  ext1139: '蝶々',
  ext1140: 'オリフラム',
  ext1141: '羽織',
  ext1142: 'お花見弁当',
  ...
  ext2136: 'ビートル',
  ext2137: 'ハチ',
  ext2138: 'Sweet Fluffy and chewy',
  ext2139: '蝶々',
  ext2140: 'オリフラム',
  ext2141: '羽織',
  ext2142: 'お花見弁当',
  ...
  ext3136: 'ビートル',
  ext3137: 'ハチ',
  ext3138: 'Sweet Fluffy and chewy',
  ext3139: '蝶々',
  ext3140: 'オリフラム',
  ext3141: '羽織',
  ext3142: 'お花見弁当',
  ...
  ext4136: 'ビートル',
  ext4137: 'ハチ',
  ext4138: 'Sweet Fluffy and chewy',
  ext4139: '蝶々',
  ext4140: 'オリフラム',
  ext4141: '羽織',
  ext4142: 'お花見弁当',
  ...
  ext5136: 'ビートル',
  ext5137: 'ハチ',
  ext5138: 'Sweet Fluffy and chewy',
  ext5139: '蝶々',
  ext5140: 'オリフラム',
  ext5141: '羽織',
  ext5142: 'お花見弁当',
...
/**
 * カードランクアップ系列 (CARD_SERIES + CARD_RARITIES から動的計算)
 *
 * 同 series 内で次 rarity (common→uncommon→rare→epic→legendary) の最初に
 * 見つかったカードを upgrade target として登録する。
 * 候補が複数ある場合は extId が「現在 ID + 1000」に最も近いものを選択
 * (MCH の命名規則: 同 series の同じ variant は基本 +1000 ずつ rarity が上がる)。
 * 上位 rarity が series 内に存在しない場合は CARD_UPGRADE_SERIES に entry なし
 (= upgrade 不可、craft / shop で表示されない)。
 */
const _RARITY_ORDER = ['common', 'uncommon', 'rare', 'epic', 'legendary'];
function _findUpgradeKey(currentKey) {
  const currentRarity = CARD_RARITIES[currentKey];
  const currentSeries = CARD_SERIES[currentKey];
  if (!currentRarity || !currentSeries) return null;
  const idx = _RARITY_ORDER.indexOf(currentRarity);
  if (idx < 0 || idx >= _RARITY_ORDER.length - 1) return null;
  const nextRarity = _RARITY_ORDER[idx + 1];
  const m = currentKey.match(/^ext(\d+)$/);
  if (!m) return null;
  const currentId = +m[1];
  const targetId = currentId + 1000;
  let best = null, bestDelta = Infinity;
  for (const k of Object.keys(CARD_SERIES)) {
    if (CARD_SERIES[k] !== currentSeries) continue;
    if (CARD_RARITIES[k] !== nextRarity) continue;
    const idMatch = k.match(/^ext(\d+)$/);
    if (!idMatch) continue;
    const delta = Math.abs(+idMatch[1] - targetId);
    if (delta < bestDelta) { bestDelta = delta; best = k; }
  }
  return best;
}
/** 全カードの upgrade target を eager 計算 (consumer は CARD_UPGRADE_SERIES[key] を読む) */
export const CARD_UPGRADE_SERIES = (() => {
  const out = {};
  for (const k of Object.keys(CARD_SERIES)) {
    const up = _findUpgradeKey(k);
    if (up) out[k] = up;
  }
  return out;
})();
